...
run(async (session, cloudId, folderId) => {
  ...
  const subNetworksResponse = await networkService.listSubnets({ networkId: network.id });
  const subNetwork = subNetworksResponse.subnets.filter(sn => sn.zoneId === TARGET_ZONE_ID).pop();
  if (!subNetwork) {
    throw new Error(`There is no subnet exists for ${TARGET_ZONE_ID} in ${network.id}`);
  }
  ...
  console.log(`Instance ${instance.id} created, ${operation.timeSpent()}ms spent.`);
});
