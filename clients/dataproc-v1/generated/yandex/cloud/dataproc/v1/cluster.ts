...
export const HadoopConfig = {
    ...
    fromJSON(object: any): HadoopConfig {
        const message = { ...baseHadoopConfig } as HadoopConfig;
        message.services = (object.services ?? []).map((e: any) => hadoopConfig_ServiceFromJSON(e));
        ...
        message.sshPublicKeys = (object.sshPublicKeys ?? []).map((e: any) => String(e));
...
export const HadoopConfig_PropertiesEntry = {
    ...
    decode(input: _m0.Reader | Uint8Array, length?: number): HadoopConfig_PropertiesEntry {
        const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseHadoopConfig_PropertiesEntry } as HadoopConfig_PropertiesEntry;
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
