...
export const RestrictionMeta = {
    ...
    toJSON(message: RestrictionMeta): unknown {
        ...
        message.valueType !== undefined &&
            (obj.valueType = restrictionMeta_RestrictionValueTypeToJSON(message.valueType));
        return obj;
    },
...
export const GetRestrictionsMetaResponse = {
    ...
    decode(input: _m0.Reader | Uint8Array, length?: number): GetRestrictionsMetaResponse {
        const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseGetRestrictionsMetaResponse } as GetRestrictionsMetaResponse;
        message.restrictionsMeta = [];
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
                    message.restrictionsMeta.push(RestrictionMeta.decode(reader, reader.uint32()));
                    break;
                default:
                    reader.skipType(tag & 7);
                    break;
