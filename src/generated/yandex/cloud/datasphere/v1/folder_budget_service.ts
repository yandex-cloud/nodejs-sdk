...
export const GetFolderBudgetResponse = {
  ...
  decode(
    ...
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = {
      ...baseGetFolderBudgetResponse,
    } as GetFolderBudgetResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  toJSON(message: SetFolderBudgetRequest): unknown {
    ...
    message.setMask !== undefined &&
      (obj.setMask = message.setMask
        ? FieldMask.toJSON(message.setMask)
        : undefined);
    message.unitBalance !== undefined &&
      (obj.unitBalance = message.unitBalance);
    message.maxUnitsPerHour !== undefined &&
...
export const FolderBudgetServiceService = {
  ...
  get: {
    ...
    responseDeserialize: (value: Buffer) =>
      GetFolderBudgetResponse.decode(value),
  },
...
export interface FolderBudgetServiceServer
...
export interface FolderBudgetServiceClient extends Client {
  ...
  get(
    ...
    callback: (
      ...
      response: GetFolderBudgetResponse
    ) => void
  ): ClientUnaryCall;
  get(
    request: GetFolderBudgetRequest,
