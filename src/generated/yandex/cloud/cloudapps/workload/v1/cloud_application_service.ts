...
export const ResolveByWorkloadRequest = {
  ...
  decode(
    ...
    const message = {
      ...baseResolveByWorkloadRequest,
    } as ResolveByWorkloadRequest;
  ...
  toJSON(message: ResolveByWorkloadRequest): unknown {
    ...
    message.workloadType !== undefined &&
      (obj.workloadType = resolveByWorkloadRequest_WorkloadTypeToJSON(
        message.workloadType
      ));
...
export const ResolveByWorkloadResponse = {
  $type:
    "yandex.cloud.cloudapps.workload.v1.ResolveByWorkloadResponse" as const,
  ...
  decode(
    ...
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = {
      ...baseResolveByWorkloadResponse,
    } as ResolveByWorkloadResponse;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
          message.cloudApplication = CloudApplication.decode(
            reader,
            reader.uint32()
          );
          break;
...
messageTypeRegistry.set(
  ResolveByWorkloadResponse.$type,
  ResolveByWorkloadResponse
);
...
export interface CloudApplicationServiceClient extends Client {
  ...
  resolveByWorkload(
    ...
    callback: (
      ...
      response: ResolveByWorkloadResponse
    ) => void
  ): ClientUnaryCall;
  resolveByWorkload(
    request: ResolveByWorkloadRequest,
