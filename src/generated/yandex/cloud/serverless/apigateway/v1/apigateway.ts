...
export const ApiGateway = {
    encode(message: ApiGateway, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
        ...
        if (message.executionTimeout !== undefined) {
            Duration.encode(message.executionTimeout, writer.uint32(130).fork()).ldelim();
        }
        return writer;
    },
    ...
    toJSON(message: ApiGateway): unknown {
        ...
        message.status !== undefined && (obj.status = apiGateway_StatusToJSON(message.status));
...
export const ApiGateway_VariablesEntry = {
    ...
    decode(input: _m0.Reader | Uint8Array, length?: number): ApiGateway_VariablesEntry {
        const reader = input instanceof _m0.Reader ? input : new _m0.Reader(input);
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseApiGateway_VariablesEntry } as ApiGateway_VariablesEntry;
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
