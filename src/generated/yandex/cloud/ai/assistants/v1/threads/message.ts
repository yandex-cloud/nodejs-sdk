...
export const MessageContent = {
  ...
  fromJSON(object: any): MessageContent {
    const message = { ...baseMessageContent } as MessageContent;
    message.content = (object.content ?? []).map((e: any) =>
      ContentPart.fromJSON(e)
    );
    return message;
  },
  ...
  toJSON(message: MessageContent): unknown {
    ...
    if (message.content) {
      obj.content = message.content.map((e) =>
        e ? ContentPart.toJSON(e) : undefined
      );
    } else {
      obj.content = [];
    }
  ...
  fromPartial<I extends Exact<DeepPartial<MessageContent>, I>>(
    object: I
  ): MessageContent {
    const message = { ...baseMessageContent } as MessageContent;
    message.content =
      object.content?.map((e) => ContentPart.fromPartial(e)) || [];
    return message;
  },
};
  ...
  encode(
    ...
    Object.entries(message.labels).forEach(([key, value]) => {
      MessageData_LabelsEntry.encode(
        {
          $type:
            "yandex.cloud.ai.assistants.v1.threads.MessageData.LabelsEntry",
          key: key as any,
          value,
        },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          break;
        case 2:
          const entry2 = MessageData_LabelsEntry.decode(
            reader,
            reader.uint32()
          );
...
export const MessageData_LabelsEntry = {
  $type:
    "yandex.cloud.ai.assistants.v1.threads.MessageData.LabelsEntry" as const,

  encode(
    message: MessageData_LabelsEntry,
    writer: _m0.Writer = _m0.Writer.create()
  ): _m0.Writer {
    if (message.key !== "") {
      writer.uint32(10).string(message.key);
  ...
  decode(
    ...
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = {
      ...baseMessageData_LabelsEntry,
    } as MessageData_LabelsEntry;
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
...
export const ContentPart = {
  ...
  fromPartial<I extends Exact<DeepPartial<ContentPart>, I>>(
    object: I
  ): ContentPart {
    const message = { ...baseContentPart } as ContentPart;
    message.text =
