...
export const YDSSource = {
  ...
  toJSON(message: YDSSource): unknown {
    ...
    if (message.supportedCodecs) {
      obj.supportedCodecs = message.supportedCodecs.map((e) =>
        ydsCompressionCodecToJSON(e)
      );
    } else {
      obj.supportedCodecs = [];
    }
    ...
    message.allowTtlRewind !== undefined &&
      (obj.allowTtlRewind = message.allowTtlRewind);
  ...
  fromPartial<I extends Exact<DeepPartial<YDSSource>, I>>(
    object: I
  ): YDSSource {
    const message = { ...baseYDSSource } as YDSSource;
