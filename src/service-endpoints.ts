...
export type ServiceEndpointsMap = Record<ServiceSlug, ServiceEndpoint[]>;

// @see https://api.cloud.yandex.net/endpoints
export const DEFAULT_SERVICE_ENDPOINTS_MAP: ServiceEndpointsMap = {
    operations: [
        {
...
export class ServiceEndpointResolver {
    private serviceEndpointsMap: ServiceEndpoint[];

    constructor(serviceEndpointsMap: ServiceEndpointsMap = DEFAULT_SERVICE_ENDPOINTS_MAP) {
        this.serviceEndpointsMap = flatten(Object.values(serviceEndpointsMap));
    }

    resolve<T extends ServiceDefinition>(generatedClientCtor: GeneratedServiceClientCtor<T>): string {
        const clientCtor = generatedClientCtor as unknown as ServiceClientConstructor;
        const { serviceName } = clientCtor;

        if (!serviceName) {
            throw new Error('Unable to retrieve serviceName of provided service client class');
        }

        const endpointItem = this.serviceEndpointsMap.find((item) => item.serviceIds.includes(serviceName));
