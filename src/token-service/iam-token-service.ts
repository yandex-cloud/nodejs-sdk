...
export class IamTokenService implements TokenService {
    ...
    private async requestToken(): Promise<cloudApi.iam.v1_iam_token_service.CreateIamTokenResponse> {
        const deadline = DateTime.now().plus({ millisecond: this.tokenRequestTimeout }).toJSDate();

        return this.client().create(
            cloudApi.iam.v1_iam_token_service.CreateIamTokenRequest.fromPartial({
